...
else
{
    ...
    if ($action == 'create')
    {
        ...
        print_fiche_titre($langs->trans("Nuevo Contacto  (cliente prospecto, cliente, proveedor)"));
        ...
        if (! empty($conf->use_javascript_ajax))
        {
            ...
            print $langs->trans("Tipo de Contacto").': &nbsp; ';
        ...
        {
            //variables de menu lateral de terceros

            $leyenda;
            if($_GET['c']=='cliente'){
                $leyenda='Nombre del Cliente';
            }
            if($_GET['a']=='acreedor'){
                $leyenda='Nombre del Acreedor';
            }
            if($_GET['p']=='proveedor'){
                $leyenda='Nombre del Proveedor';
            }

            print '<tr><td><span span id="TypeName" class="fieldrequired">'.$leyenda.'</span></td><td'.(empty($conf->global->SOCIETE_USEPREFIX)?' colspan="3"':'').'><input type="text" size="30" maxlength="60" name="nom" value="'.$object->name.'"></td>';
            if (! empty($conf->global->SOCIETE_USEPREFIX))  // Old not used prefix field
            {
                print '<td>'.$langs->trans('Prefix').'</td><td><input type="text" size="5" maxlength="5" name="prefix_comm" value="'.$object->prefix_comm.'"></td>';
            }
        ...
        // Prospect/Customer
        //sprint '<tr><td width="25%"><span class="fieldrequired">'.$langs->trans('ProspectCustomer').'</span></td><td width="25%"><select class="flat" name="client">';
...
//Validacion para codigo de cliente
  $texto;
  if($_GET['c']=='cliente'){
    $texto= $langs->trans('CustomerCode');  
  }
   if($_GET['a']=='acreedor'){  
    $texto= "Codigo de Acreedor"; 
  }
        print '<td width="25%">'.$texto.'</td><td'.(empty($conf->global->SOCIETE_USEPREFIX)?' colspan="3"':'').'>';
        ...
        // Address
        print '<tr><td valign="top">'.$langs->trans('Calle').'</td><td><input name="address" cols="40" rows="3" wrap="soft">';
        print $object->address;
        print '</input></td>';
        print '<td>'.$langs->trans('Numero de Calle').'</td><td><input type="text" name="fax" value="'.$object->fax.'"></td></tr>';
       

        // Zip / Town
        print '<tr><td>'.$langs->trans('Zip').'</td><td>';
        print $formcompany->select_ziptown($object->zip,'zipcode',array('town','selectcountry_id','departement_id'),6);
        print '</td><td>'.$langs->trans('Colonia').'</td><td>';
        print $formcompany->select_ziptown($object->town,'town',array('zipcode','selectcountry_id','departement_id'));
        print '</td></tr>';


        // atributos agregados desde dolibarr (Delegacion/Municipio)
        ...
        {
            print '<tr><td>'.$langs->trans('Estado').'</td><td colspan="3">';
        ...
        // Email web
        print '<tr><td>'.$langs->trans('EMail').(! empty($conf->global->SOCIETE_MAIL_REQUIRED)?'*':'').'</td><td><input type="text" name="email" size="32" value="'.$object->email.'"></td></tr>';
        print '<tr><td>'.$langs->trans('Web').'</td><td colspan="3"><input type="text" name="url" size="32" value="'.$object->url.'"></td></tr>';

        // Phone / Cel
        print '<tr><td>'.$langs->trans('Telefono local').'</td><td><input type="text" name="phone" value="'.$object->phone.'"></td>';
        print '<td>'.$langs->trans('Celular').'</td><td><input type="text" name="cel" value="'.$object->cel.'"></td></tr>';
        ...
        /*print '</td>';
        //print '<td class="nowrap">'.$langs->trans('wwwwwwwwwwwwwwwwwwwww').'</td>';
        ...
        if (empty($conf->global->MAIN_DISABLEVATCHECK))
        {
            ...
            if (! empty($conf->use_javascript_ajax))
            {
                ...
                //print "newpopup('".DOL_URL_ROOT."/societe/checkvat/checkVatPopup.php?vatNumber='+a,'".dol_escape_js($langs->trans("aaaaaaaaaa"))."',500,300);\n";
                print "}\n";
                print '</script>';
                print "\n";
                //$s.='<a href="#" class="hideonsmartphone" onclick="javascript: CheckVAT(document.formsoc.tva_intra.value);">'.$langs->trans("sssssss").'</a>';
                $s = $form->textwithpicto($s,$langs->trans("VATIntraCheckDesc",$langs->trans("VATIntraCheck")),1);
            }
            else
            {
                $s.='<a href="'.$langs->transcountry("VATIntraCheckURL",$object->country_id).'" target="_blank">'.img_picto($langs->trans("ddddd"),'help').'</a>';
            }
        }
        print $s;
        print '</td>';*/
        ...
        // Type - Size
        print '<tr><td>'.$langs->trans("Tipo de Contacto").'</td><td>'."\n";
        print $form->selectarray("typent_id",$formcompany->typent_array(0), $object->typent_id);
        if ($user->admin) print info_admin($langs->trans("YouCanChangeValuesForThisListFromDictionnarySetup"),1);
        print '</td>';
        /*print '<td>'.$langs->trans("sasa").'</td><td>';
    ...
    else
    {
        ...
        */
        ...
        // Name
        print '<tr><td width="25%">'.$langs->trans('Nombre del Acreedor').'</td>';
        print '<td colspan="3">';
        ...
        {
            print '<tr><td>';
            print $langs->trans('Codigo Acreedor').'</td><td colspan="'.(2+(($showlogo || $showbarcode)?0:1)).'">';
            print $object->code_client;
        ...
        print '<tr><td>'.$langs->trans('Cel').'</td><td style="min-width: 25%;">'.dol_print_phone($object->cel,$object->country_code,0,$object->id,'AC_CEL').'</tr></td>';
        ...
        print '<tr><td>'.$langs->trans("Tipo de Acreedor").'</td><td>'.$object->typent.'</td><td>'.$langs->trans("Staff").'</td><td>'.$object->effectif.'</td></tr>';
