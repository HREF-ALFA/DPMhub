...
function print_eldy_menu($db,$atarget,$type_user,&$tabMenu,&$menu,$noout=0)
{
	...
	if ($showmode)
	{
		...
		if (empty($noout)) print_start_menu_entry($idsel,$classname,$showmode);
		if (empty($noout)) print_text_menu_entry($langs->trans("Directorio"), $showmode, DOL_URL_ROOT.'/societe/index.php?mainmenu=companies&amp;leftmenu=', $id, $idsel, $classname, $atarget);
		if (empty($noout)) print_end_menu_entry($showmode);
		$menu->add('/societe/index.php?mainmenu=companies&amp;leftmenu=', $langs->trans("Directorio"), 0, $showmode, $atarget, "companies", '');
	}
...
function print_left_eldy_menu($db,$menu_array_before,$menu_array_after,&$tabMenu,&$menu,$noout=0,$forcemainmenu='',$forceleftmenu='')
{
	...
	if ($mainmenu)
	{
		...
		if ($mainmenu == 'companies')
		{
			...
			{
				$langs->load("companies");
				$newmenu->add("/societe/index.php?leftmenu=thirdparties", $langs->trans("Directorio"), 0, $user->rights->societe->lire, '', $mainmenu, 'thirdparties');

				if ($user->rights->societe->creer)
				{
					//$newmenu->add("/societe/soc.php?action=create", $langs->trans("Nuevo Contacto"),1);
			...
			{
				$langs->load("commercial");
				$newmenu->add("/comm/prospect/list.php?leftmenu=prospects", $langs->trans("Listado de Acreedores"), 1, $user->rights->societe->lire, '', $mainmenu, 'prospects');
				...
				$newmenu->add("/societe/soc.php?leftmenu=prospects&amp;action=create&amp;type=p&a=acreedor", $langs->trans("Nuevo Acreedor"), 2, $user->rights->societe->creer);
			...
			{
				$langs->load("commercial");
				$newmenu->add("/comm/list.php?leftmenu=customers", $langs->trans("Listado de Clientes"), 1, $user->rights->societe->lire, '', $mainmenu, 'customers');

				$newmenu->add("/societe/soc.php?leftmenu=customers&amp;action=create&amp;type=c&c=cliente", $langs->trans("Nuevo Cliente"), 2, $user->rights->societe->creer);
			...
			{
				$langs->load("suppliers");
				$newmenu->add("/fourn/liste.php?leftmenu=suppliers", $langs->trans("Listado de Proveedores"), 1, $user->rights->fournisseur->lire, '', $mainmenu, 'suppliers');

				if (empty($user->societe_id))
				{
					$newmenu->add("/societe/soc.php?leftmenu=suppliers&amp;action=create&amp;type=f&p=proveedor",$langs->trans("Nuevo Proveedor"), 2, $user->rights->societe->creer && $user->rights->fournisseur->lire);
				}
			...
			if (! empty($conf->categorie->enabled))
			{
				...
				// Categories acreedores
				$newmenu->add("/categories/index.php?leftmenu=cat&amp;type=3", $langs->trans("Categorias Acreedores"), 0, $user->rights->categorie->lire, '', $mainmenu, 'cat');
				if (empty($user->societe_id))
				{
		...
		//Menu de Importacion
		if ($mainmenu == 'Importacion')
		{
		// Societes
		if (! empty($conf->societe->enabled))
		{
			$langs->load("Importacion");
			$newmenu->add("/importacion/index.php?leftmenu=thirdparties", $langs->trans("Importacion"), 0, $user->rights->societe->lire, '', $mainmenu, 'thirdparties');

			
				$newmenu->add("/importacion/facture/index.php?leftmenu=suppliers_bills", $langs->trans("Factura de proveedor"),1);
				$newmenu->add("/importacion/trans_int/fiche.php?action=create", $langs->trans("Transito internacional"),1);
				$newmenu->add("/importacion/fiche.php?action=create", $langs->trans("Frontera"),1);
				$newmenu->add("/importacion/fiche.php?action=create", $langs->trans("Importacion"),1);
				$newmenu->add("/importacion/fiche.php?action=create", $langs->trans("Transito nacional"),1);
				$newmenu->add("/importacion/fiche.php?action=create", $langs->trans("Entrada a almacen"),1);
